...
function ViewFileLink() {
  ...
  const [userPassword, setUserpassowrd] = useState(null);
  ...
  const fetchData = async () => {
    try {
      ...
      setUserpassowrd(response.data.data.file.user.password);
  ...
  return (
    <Box>
      {!isPasswordCorrect ? (
        <Modal isOpen={!isPasswordCorrect} onClose={() => {}}>
          <ModalOverlay />
          <ModalContent>
            <ModalHeader style={{ color: "#b19552" }}>
              Password Protected
            </ModalHeader>
            <ModalBody>
              <Input
          ...
          {loading ? (
            ...
            <Flex direction="column">
              <Card overflowX={{ sm: "scroll", xl: "hidden" }}>
                <CardBody style={{ padding: "00px" }} className="cardss">
                  <FormLabel
