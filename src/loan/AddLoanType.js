...
function AddLoanType() {
    ...
    useEffect(() => {
        const secondLastSubtype = formData.loan_subtype[formData.loan_subtype.length - 2];
        const lastSubtype = formData.loan_subtype[formData.loan_subtype.length - 1];
        if (secondLastSubtype !== "" && lastSubtype !== "") {
            handleAddSubtype();
        } else if (secondLastSubtype === "" && lastSubtype === "" && formData.loan_subtype.length > 1) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const data = {
                ...
                loan_type: formData.loan_subtype.slice(0, formData.loan_subtype.length - 1)
            }

            if (isSubtype && formData.loan_subtype.slice(0, formData.loan_subtype.length - 1).length == 0) {
                    ...
                    <CardBody>
                        <form onSubmit={handleSubmit}>
                            ...
                            <FormControl id="loan_type" mt={4} isRequired>
                                ...
                                <Input name="loan_type" onChange={handleChange} value={formData.loan_type} />
                            </FormControl>
                            ...
                            <FormControl display='flex' alignItems='center' mt="5">
                                ...
                                <Switch id='email-alerts' onChange={handleSwitchChange} isChecked={isSubtype} />
                            </FormControl>

                            {isSubtype && formData.loan_subtype.map((subtype, index) => (
                                <FormControl key={index} id={`loan_subtype_${index}`} mt={8}>
                                    ...
                                    <Flex alignItems="center">
                                        <Input
                                            ...
                                            value={subtype}
                                            ref={(inputRef) => (subtypeRefs.current[index] = inputRef)}
                                            mr={2} // Add some margin to the right of the input
                                        />
                                        <Button type="button" onClick={() => handleRemoveSubtype(index)} colorScheme="red">
                                            Remove
                                        </Button>
                                    </Flex>
                                </FormControl>
